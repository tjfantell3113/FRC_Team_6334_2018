...
public class autoRightToRightSwitch extends CommandGroup {
    ...
    public autoRightToRightSwitch() {
        ...
        addSequential(new moveDistanceX(90, 0.7));	//Move forward
        addSequential(new moveDistanceX(5, -0.3), 1);	//Decelerate
        addSequential(new moveDistanceX(5, 0.3), 1);	//Decelerate
        addParallel(new autoLift(RobotMap.switchHeight));
        addSequential(new moveDistanceX(5, -0.3), 1);	//Decelerate
        addSequential(new moveDistanceX(5, 0.3), 1);	//Decelerate
        addSequential(new autoTurn(-85, 0.35, -1));	//Turn to switch
        addSequential(new moveDistanceX(36, 0.35), 2);	//Move to switch
        addSequential(new autoIntake(true, 1));	//Eject the box
